...
bool is_position_in_shape(struct Hex pos) {
    for (int i = 0; i < mydata->lattice_shape_size; i++) {
        if (mydata->lattice_shape[i].q == pos.q && mydata->lattice_shape[i].r == pos.r ) {
            return true;
        }
    }
    return false;
...
bool is_out_position_in_shape(struct Hex pos) {
    for (int i = 0; i < mydata->lattice_shape_size; i++) {
        if (mydata->lattice_shape[i].q == pos.q - 2 && mydata->lattice_shape[i].r == pos.r + 5) {
            return true;
        }
    }
    return false;
...
struct Hex find_nearest_unoccupied_target_distributed() {
    ...
    for (int dq = -COMM_RANGE; dq <= COMM_RANGE; dq++) {
        for (int dr = -COMM_RANGE; dr <= COMM_RANGE; dr++) {
            ...
            // 跳过超出通信范围的点
            //if (hex_distance(self_q, self_r, q, r) > 2) continue;
            ...
            // 不在形状内
            if(!is_position_in_shape((struct Hex){self_q,self_r}) && is_position_in_shape((struct Hex){q, r})){
...
bool detect_immediate_conflict_enhanced() {
    ...
    for (int i = 0; i < mydata->N_Neighbors; i++) {
        ...
        if(mydata->neighbors[i].has_movement_intent == 1){
            // 邻居id更小
            if (mydata->neighbors[i].ID < kilo_uid && (mydata->neighbors[i].hex_r >= mydata->hex_r)) {

                double dist_to_target = hex_distance(my_start_hex.q, my_start_hex.r, 
                                               mydata->neighbors[i].hex_q, 
                                               mydata->neighbors[i].hex_r);
                // 如果和邻居距离过小就不允许移动
                // 如果目标点在邻居的通信范围内
                if(dist_to_target <= 1.0)  // 使用你的通信范围，比如2格
                {
                    printf("当前机器人 %d 和邻居 %d 距离过近，放弃。\n", kilo_uid, mydata->neighbors[i].ID);
                    return true;
        ...
        if (mydata->neighbors[i].intended_target_q == mydata->intended_target_q &&
            ...
            struct Cartesian n_start_position = hex_to_Cart(n_start_hex);
            struct Cartesian n_target_position = hex_to_Cart(n_target_hex);
